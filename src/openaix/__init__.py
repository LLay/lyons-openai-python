...
# openaix._exceptions.NotFoundError -> openaix.NotFoundError
__locals = locals()
for __name in __all__:
    if not __name.startswith("__"):
        try:
            __locals[__name].__module__ = "openaix"
...
_ApiType = _te.Literal["openaix", "azure"]
...
class _AmbiguousModuleClientUsageError(OpenAIError):
    def __init__(self) -> None:
        super().__init__(
            "Ambiguous use of module client; please set `openaix.api_type` or the `OPENAI_API_TYPE` environment variable to `openaix` or `azure`"
        )
